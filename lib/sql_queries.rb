...
"SELECT projects.title, (SUM(pledges.amount)-projects.funding_goal) AS funded FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING funded >=0;"
end
...
"SELECT projects.category, SUM(pledges.amount) FROM project JOIN pledges ON projects.id = pledges.project_id WHERE project.category='books';"
end
